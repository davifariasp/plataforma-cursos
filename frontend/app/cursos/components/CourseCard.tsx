...
export function CourseCard({ curso, inscricoes, onEnroll }: CourseCardProps) {
  ...
  const isEnrolled = inscricoes.find((inscricao: any) => inscricao === curso.idCurso);
  ...
  return (
    <Card className="p-6">
      ...
      { (curso.numeroVagas > 0) && (curso.numeroInscricoes <= curso.numeroVagas) && !isEnrolled && (
        <Button onClick={() => onEnroll(curso.idCurso)} className="w-full">
          Inscrever
        </Button>
      )}
      {isEnrolled && (
        <p className="text-green-600 text-center">Inscrito</p>
      )}
      {curso.numeroVagas == 0 && !isEnrolled && (
